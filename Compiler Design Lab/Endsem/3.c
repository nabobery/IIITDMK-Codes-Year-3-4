...
// (b) Write a C Program to identify and print semantic error as 
// Error: An UDV Abcd@7 is used before its declaration
// Error: Data types of LHS and RHS of assignment statements are not matching
...
int increment_token(char *token)
{
    ...
    if (flag == true)
    {
        ...
        Tokens[count_udv].declared = false;
        index = count_udv;
        count_udv++;
    }
    return index;
    ...
    while (right <= len && left <= right)
    {
        ...
        else if (isDelimiter(str[right]) == true && left != right || (right == len && left != right))
        {
            ...
            else if (validUDV(subStr) == true)
            {
                ...
                if(validUDV(temp) == true){
                    int in = findToken(temp);
                    if(Tokens[ind].declared == false){
                        printf("Error: An UDV %s is used before its declaration\n",temp);
                        err = true;
                        break;
                    }
                    if(strcmp(Tokens[ind].datatype, Tokens[in].datatype) != 0){
                        printf("LHS = %s, DataType = %s\n",subStr);
                        printf("RHS = %s",temp);
                        printf("Data Types of LHS and RHS of assignment statements are not matching!\n");
                        err = true;
                        break;
                    }
                }
                else{
                    if(declared == false && Tokens[ind].declared == false){
                        printf("Error: An UDV %s is used before its declaration\n",subStr);
                        err = true;
                        break;
                    }
                    else{
                        Tokens[ind].declared = declared;
                        strcpy(Tokens[ind].datatype, dt);
                    }
                }
            }
            left = right;
