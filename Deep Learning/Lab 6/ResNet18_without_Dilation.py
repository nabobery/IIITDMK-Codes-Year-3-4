...
def ResNet18_Without_Dilation(inputShape=(224,224,3), num_classes=8):
    ...
    x = convolutional_block(x, filters=128, kernel_size=3, stride=2)
    x = identity_block(x, filters=128, kernel_size=3)

    x = convolutional_block(x, filters=256, kernel_size=3, stride=2)
    x = identity_block(x, filters=256, kernel_size=3)

    x = convolutional_block(x, filters=512, kernel_size=3, stride=2)
