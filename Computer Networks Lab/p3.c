...
2. Receive message:
    You need to take input as <Data bits>+<VRC code bits> from user. Then, perform VRC and display <Data bits> and detection message (Accept/Reject)
*/
...
int main()
{
    ...
    while (choice != 'q')
    {
        ...
        else if (choice == 'b')
        {
            ...
            scanf("%d", &v);
            if ((n - v) % 4 != 0)
                printf("Something is wrong about the size of data bits. Please try again\n");
            else
            {
                printf("Please enter the data bits of the message: \n");
                char result[n - v];
                ...
                if (flag == false)
                    printf("The Given Data is not corrupted (Accepted) \n");
                else
                    printf("The Given Data is corrupted (Rejected) \n");
            }
        }
        else if (choice == 'q')
        {
